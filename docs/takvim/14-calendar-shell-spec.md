...
| **BR-SHELL-45** | ⚠️ **Önizleme kartı salt okunurdur; aksiyon taşımaz.** Etkinlik üzerindeki işlemler **bağlam menüsüne** (BR-SHELL-46) ve etkinlik yüzeyine aittir. *(2 Eylül 2026'da bu hâline döndürüldü: 1 Eylül'de karta silme eklenmişti, bağlam menüsü kapsama girince yıkıcı aksiyon tek yerde toplandı — SR-SHELL-12.)* **Bilgi yüzeyi ile aksiyon yüzeyi ayrıdır.** |
| **BR-SHELL-46** | ⭐ **Etkinlikte bağlam menüsü** *(`KEEP-13` — mevcut üründe bulunan kalıp)*. Izgaradaki bir etkinliğe sağ tıklandığında **düzenle · oda değiştir/seç · sil** aksiyonlarını taşıyan bir menü açılır. Menü **hızlandırıcıdır**: buradaki her işlem etkinliğe tıklayınca açılan yüzeyden de yapılabilir. |
| **BR-SHELL-47** | Menü **yalnızca kullanıcının düzenleyebildiği etkinliklerde** açılır. Paylaşılan salt okunur etkinlikte **hiç açılmaz** ve tarayıcının kendi menüsü engellenmez — `12` BR-CAL-27. |
| **BR-SHELL-48** | ⚠️ Pasif bir menü öğesinin **sebebi menüde yazılı olur** (`UX-54`'ün çözümü, `11` ST-DIS-02). Sonucu olan ama engellenmeyen aksiyonlarda **sonuç önceden belirtilir** — ör. bekleyen talebi olan etkinlikte oda değiştirmenin talebi iptal edeceği (`18` BR-APR-33). |
| **BR-SHELL-49** | Menüden silme **doğrudan gerçekleşmez**: açık onay diyaloğundan geçer ve etkilenecek rezervasyon sonucunu belirtir — `11` ST-DES-01/02, `15` BR-EVT-29/30. |
| **BR-SHELL-50** | Bağlam menüsü **dokunmatik cihazlarda karşılığı olmayan bir hızlandırıcıdır**; mobilde bulunmaz. Bu nedenle hiçbir işlem yalnızca menüden erişilebilir olamaz (BR-SHELL-46). |
...
| SR-SHELL-12 | Etkinlik üzerinde kaç yüzey olmalı? | **Hover = oku, sağ tık = yap, tıklama = tam düzenleme.** Önizleme kartı salt okunura döndürüldü; yıkıcı aksiyon bağlam menüsünde toplandı (BR-SHELL-45/46) | ⚠️ Sağ tık kalıbı mevcut üründe **vardı ama FAZ 1'de kaydedilmemişti** (`KEEP-13`, rev.4). Kaldırmak taşınan kullanıcı için regresyon olurdu. Kalıp kapsama girince aynı etkinlik için üç yüzey oluştu; bilgi ile aksiyonun ayrılması en az çakışan bölünme. Menü hızlandırıcı olduğu için mobilde karşılığı yok — bu yüzden hiçbir işlem yalnız menüden erişilebilir değil |
